...
def anderson_solver(f, z_init,y, m=5, lam=1e-4, max_iter=10, tol=1e-5, beta=1.0):
    # with torch.no_grad():
        # x0 = z_init
        # x0_new,x1 = f(x0,y)
        ...
        # X_out=np.zeros((16,5,100))
        # output_exp=np.zeros((16,100))
        # for i in range(16):
        ...
        #         if res < tol:
        #             break
        #     X_out[i]=X[:,k%m].view_as(x0)
        # x0=z_init
        z_init = torch.t(z_init)
        x0=z_init
        y=torch.t(y)
        # print(x0.shape)
        bsz, d= z_init.shape
        X = torch.zeros(bsz, 5, d*1, dtype=x0.dtype, device=x0.device)
        F = torch.zeros(bsz, 5, d*1, dtype=x0.dtype, device=x0.device)
        x0_new,x1=f(x0,y)
        # x0_new_t=torch.t(x0_new)
        X[:,0], F[:,0] = x0.view(bsz, -1), x0_new.view(bsz, -1)
        out_exp,output=f(F[:,0].view_as(x0),y)
        X[:,1], F[:,1] = F[:,0], out_exp.view(bsz, -1)
        
        H = torch.zeros(bsz, 6, 6, dtype=x0.dtype, device=x0.device)
        H[:,0,1:] = H[:,1:,0] = 1
        Y = torch.zeros(bsz, 6, 100, dtype=x0.dtype, device=x0.device)
        Y[:,0] = 1
        # print(H.shape)
        res = []
        for k in range(2, max_iter):
            ...
            X[:,k%5] = beta * (alpha.transpose(1,2) @ F[:,:n])[:,0] + (1-beta)*(alpha.transpose(1,2) @ X[:,:n])[:,0]
            # print(X.type)
            F[:,k%5],_ = f(X[:,k%5].view_as(x0).long(),y.long())#.view(bsz, -1)
            res.append((F[:,k%5] - X[:,k%5]).norm().item()/(1e-5 + F[:,k%5].norm().item()))
            if (res[-1] < tol):
                break
        return X[:,k%5].view_as(x0), res
